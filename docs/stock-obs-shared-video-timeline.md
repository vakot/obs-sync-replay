...
The probe source uses only public APIs common to stock OBS 32.2.1 and does not
reference an association callback or patched encoder API. The pinned SDK was
rebuilt for this branch from the verified official 32.2.1 archive; the resulting
headers and libobs source contain no input-association symbols. The runtime log and
the build therefore provide clean stock-API provenance for this experiment.
...
This branch uses the official 32.2.1 source archive and the public stock API surface;
no association patch or custom encoder implementation is part of the build.
...
Evidence log: [`2026-08-21 17-25-28.txt`](../obs-dev/config/obs-studio/logs/2026-08-21%2017-25-28.txt).

The clean three-second run selected:

```text
commonStartCTS = 23541078340476
commonEndCTS   = 23543628340374
first CTS A/B  = 23541078340476 / 23541078340476
last CTS A/B   = 23543628340374 / 23543628340374
CTS mismatches = 0
head packets   = 60 / 60
tail packets   = 0 / 0
muxed packets  = 154 / 154
```

Independent ffprobe validation reported 154 decoded frames in each file, duration
2.550000 seconds in each file, and identical first/last decoded presentation
...
```text
commonStartCTS = 23547245006896
commonEndCTS   = 23549995006786
first CTS A/B  = 23547245006896 / 23547245006896
last CTS A/B   = 23549995006786 / 23549995006786
CTS mismatches = 0
head packets   = 60 / 60
tail packets   = 0 / 0
muxed packets  = 164 / 164
```

Independent ffprobe validation reported 164 decoded frames in each file, duration
2.750000 seconds in each file, and identical first/last decoded presentation
...
| --- | ---: | ---: | --- | --- | ---: |
| x264 | 154 / 154 | 2.550000 / 2.550000 s | DTS monotonic; B-frame PTS order expected | passed | 0 |
| NVENC | 164 / 164 | 2.750000 / 2.750000 s | DTS monotonic; B-frame PTS order expected | passed | 0 |
...
The x264 run retained 3,468,038 compressed payload bytes across both streams,
including 901,370 bytes of startup pre-roll. NVENC retained 3,733,344 bytes,
including 1,000,012 bytes of startup pre-roll. The selected range itself was
154 or 164 packets per stream. The POC performs two stock video encodes, packet
...
- The runtime uses a disposable portable OBS profile and the pinned official OBS
  32.2.1 SDK; the generated output files remain ignored local evidence rather than
  release artifacts.
