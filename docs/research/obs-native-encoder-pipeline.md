...
### PR #7 — patched-OBS end-to-end association validation (2026-08-20)

The patched OBS 32.2.1 runtime and the current plugin were deployed together and
exercised at 1920x1080/60 with a 40 ms graphics delay every 600 master frames.
Both encoders ran past the required ten real lag events:

* `obs_nvenc_h264_tex`: 12+ lag boundaries. At every preserved CTS-alias boundary,
  A and B packet callbacks recovered the same `logical_slot_id`, `source_frame_id`,
  `request_id`, packet CTS, and local encoder PTS even when callback order differed.
  The packet-set records retain the alias; no CTS repair, callback-order inference,
  or PTS offset was applied. The ordered A/B logical-slot sets remained equal and
  normal cadence resumed after each event.
* `obs_x264`: 14 lag boundaries with zero CTS aliases, zero deferred/missing
  associations, and 45 successful A/B single-packet validations. The same
  submission hook works through the raw path unchanged.

The first x264 deployment exposed and fixed one propagation omission in the patched
runtime: `output_frame()` now copies `vframe_info.source_frame_id` into the raw
`video_data` before `video_output_lock_frame2`. The rerun is the acceptance result;
the earlier `source_frame_id=0` run is not treated as passing evidence.

Program scene switching was exercised through the OBS frontend while the x264
experiment was active (`Gameplay Test` → `Camera Test` → `Combined Reference` →
`Gameplay Test`). Each request succeeded, and A/B validation continued with no
association error or drift. This is runtime evidence only; Replay Buffer frontend
integration is not implemented.

These runs prove that the proposed hook closes the encoder-association blocker
*technically* for both raw and texture encoders on the pinned patched OBS 32.2.1
runtime. It is still not a production contract until the hook is upstreamed (or an
equivalent supported API is provided).
...
- Repeat the runtime matrix on additional GPUs/driver versions before treating the
  private hook as production-ready.
...
- Obtain or upstream the generic input-association hook before implementing the
  production replay adapter. The public video-only output lifecycle is resolved,
  while the current private dependency remains a release gate.
