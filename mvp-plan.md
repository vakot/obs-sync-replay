...
Build a minimal OBS Studio plugin capable of recording/replay-buffering independent
OBS scene streams into separate video files while guaranteeing frame-for-frame
synchronization between every participating output.

Phase 7 topology direction supersedes the fixed-name/fixed-cardinality startup
assumption for production capture: the active stream set is explicit Master/Program
plus every eligible real scene in the current OBS scene collection. The two-scene
examples below remain the minimum synchronization fixture and output contract;
production discovery must not create or require named Scene A/Scene B placeholders.
...
* OBS Studio plugin.
* Windows first.
* At least two eligible OBS scenes, discovered from the active collection.
...
* Per-scene settings UI beyond the current all-scenes topology.
* Streaming.
* macOS/Linux support.
* Advanced UI.
...
Plugin loads correctly in OBS and discovers Master plus every eligible real scene
through public OBS APIs, with stable identity and deterministic collection order.
