...
std::unique_ptr<obs_sync_replay::ThreeStreamCapturePoc> three_stream_capture_poc;
...
void StopPluginOwnedPipeline(const char* boundary) {
    ...
    if (three_stream_capture_poc) {
        three_stream_capture_poc->Stop();
    }
...
void RequestPluginOwnedPipelineStop(const char* boundary) {
    ...
    if (!three_stream_capture_poc && !master_frame_coordinator && !synchronized_scene_renderer &&
...
void OnFrontendEvent(enum obs_frontend_event event, void *) {
    ...
    three_stream_capture_poc = std::make_unique<obs_sync_replay::ThreeStreamCapturePoc>(
        obs_sync_replay::kResearchSceneAName, obs_sync_replay::kResearchSceneBName);
    three_stream_capture_poc->Start();
...
void obs_module_unload(void) {
    ...
    three_stream_capture_poc.reset();
